# web-monitor

![Version](https://img.shields.io/badge/version-2.4.1-blueviolet)
![Node.js](https://img.shields.io/badge/Node.js-339933?logo=node.js&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)
![License](https://img.shields.io/badge/license-MIT-blue)
![PRs Welcome](https://img.shields.io/badge/PRs-welcome-orange)
![Maintained by](https://img.shields.io/badge/Maintained_by-shubhyagami-blue)
![Commit Activity](https://img.shields.io/github/commit-activity/m/shubhyagami/web-monitor)
![Stars](https://img.shields.io/github/stars/shubhyagami/web-monitor)

A real-time web monitoring dashboard built with Node.js to monitor your application's performance, track endpoint uptime, visualize response times, and set custom alerts for performance degradation.

## Features

- **Real-time Dashboard:** Visualize live metrics for all monitored URLs.
- **Interactive Latency Heatmaps:** Identify slow endpoints instantly with color-coded grids and hover-over details.
- **Custom Alerts:** Integrate Slack or email notifications for defined response-time thresholds.
- **Theming Support:** Customize dashboard colors and layout by creating a `theme.json` file in the `config/` directory.
- **Log Rotation:** Enable log rotation by setting `LOG_ROTATION=true` to manage historical data.

## Getting Started

To start monitoring your application, follow these steps:

1.  Clone the repository and navigate into the directory:
    ```
    git clone https://github.com/shubhyagami/web-monitor.git
    cd web-monitor
    ```
2.  Install dependencies:
    ```
    npm install
    ```
3.  Start the development server:
    ```
    npm run dev
    ```
4.  Open `http://localhost:3000` in your browser to view the live metrics dashboard.

## Key Concepts and Best Practices

- **Dynamic Secrets:** Reference environment variables in `theme.json` using `$VAR` syntax.
- **Log Pruning:** Set `LOG_ROTATION=true` to automatically prune old logs and keep data storage lean.

## Example Usage

Imagine deploying `web-monitor` as an internal health check for your customer-facing SaaS product. Configure it to poll critical endpoints every 30 seconds, set response-time thresholds, and connect the event stream to a Slack webhook.

## Contributing

Contributions are welcome! Before opening a pull request, follow these steps:

1.  **Open an Issue:** Describe the bug or feature request to get feedback and a case number.
2.  **Fork and Branch:** Fork the repo, create a feature branch from `main` (e.g., `git checkout -b feat/your-idea`), and run tests.
3.  **Run Tests:** Ensure all tests pass across supported Node versions (18, 20, 22) by running:
    ```bash
    npm test
    ```
4.  **Submit a PR:** Open a pull request against the `main` branch with a clear description of your changes.

May your response times be low and your uptime high.
